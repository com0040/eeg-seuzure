# -*- coding: utf-8 -*-
"""
Created on Wed Jul 27 10:21:57 2022
...
pid = [splitCSV[i][2][34:42] for i in range(0,len(file_name_list))]
# 분리된 것 중 pid만 저장
...
for e in range (0,len(pid_without_overrap)):
    ...
    pidpath = ['%s' %(file_name_list[f]) for f in range(
        0,len(pid)) if pid_without_overrap[e] == pid[f]]
    # 같은 pid경로를 리스트로 저장
    pidpath.sort() # 한 pid에 대한 경로의 리스트를 오름차순으로 정리
    j=1 # 주기용 j
    print('총 %s 중 %s번째 PID:%s 파일수:%s' %(
        len(pid_without_overrap[e]),e+1,pid_without_overrap[e],len(pidpath)), end=' ')
    ...
    for h in tqdm(range(0,len(pidpath)),desc=('PID:%s' %pid_without_overrap[e])):
        df = pd.read_csv(pidpath[h], names = ['1', '2', '3'], end=' ')
