# -*- coding: utf-8 -*-
"""
Created on Mon Jul 11 13:00:31 2022
...
def edf_to_npy(file_path_list, output_path):
    sigbufs_list =[]
    for k in tqdm(file_path_list, desc='edf -> sigbuf -> np -> lsit에 저장'):
        ...
        if len(sigbufs[0])%4096 !=0: # 배열을 4096크기로 나누어 떨어지지 않을때
             sigbufs_divide_1 = sigbufs[:,0:(len(sigbufs[0])//4096)*4096]
             # 
    ...
    np.save('%sedf_to_npy' % output_path, all_sigbufs) # .npy 파일로 저장
...
file_path_list= load_file_path(file_path)
edf_to_npy(file_path_list, output_path)
