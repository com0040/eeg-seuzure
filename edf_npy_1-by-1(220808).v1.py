# -*- coding: utf-8 -*-
"""
Created on Mon Aug  8 14:05:11 2022
...
#%%           
    ...
    sigbufs = sigbufs[:,0:(len(sigbufs[0])//4096)*4096] # 배열을 4096크기로 나누어 떨어지지 않을 지 모르니
    sigbufs_label = sigbufs_label[0,0:(len(sigbufs_label[0])//4096)*4096] # 라벨도 마찬가지
        
    for a in range(len(num_of_arr)): # 데이터 길이만큼 4096만큼 자를거임
        sigbufs_label_arr[a,0,0:4096] = sigbufs_label[0,a*4096:a*4096+4096] 
...
#%%              
    ...
    '''
#%%
    # for i in range(len(seizure_path)): # 인덱스만큼 반복
        # sigbufs_last[:,:,i] = sigbufs[:,:,[i]] 
    ...
    np.save('%s/%s/%s' % (output_path, k[125:133], k[125:133]), sigbufs_arr) # .npy 파일로 저장
    np.save('%s%s/%s_label' % (output_path, k[125:133], k[125:133]), sigbufs_label_arr)
    # .npy 파일로 저장    
