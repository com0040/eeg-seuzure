# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 10:09:45 2022
...
for k in file_path_list: # 변환 과정을 한눈에 알아보자 
    print('%d번째 edf --> .npy파일 생성 시작' %(g+1), end=' ')
    ...
    for t in file_seizure: # seizure 있는지 확인하고 sigbufs_label에 표기
        if t[125:138] == k[125:]: # seizure 있으면 
    ...
    if len(sigbufs[0])%4096 !=0: # 배열을 4096크기로 나누어 떨어지지 않을때
        ...
        sigbufs_label = sigbufs[:,0:(len(sigbufs[0])//4096)*4096]
        # 4096으로 나눠서 나머지 없에고 갯수 맞춘 후
        sigbufs = sigbufs.reshape(23,4096,-1) 
        sigbufs_label = sigbufs.reshape(1,4096,-1) 
        # 3차원 모양으로 바꿈
    else:
        sigbufs_label = sigbufs.reshape(1,4096,-1) 
        sigbufs = sigbufs.reshape(23,4096,-1)
        # 나누어 떨어지면 그냥 바꿈
    print('배열모양 ', sigbufs.shape)
